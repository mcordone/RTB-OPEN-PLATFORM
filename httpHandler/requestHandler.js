	...
	if(undefined != exchConfigData){
		getExchangeRequestBidData(req, res, function(error, data){
			if (error) {
				...
				responseHandler.sendNoBidResponse(res);
			...
			console.log('exchConfigData.name ' + exchConfigData.name);
			var exchAdapter = ExchangeManager.getExchangeAdaptorInst(exchConfigData.name);
			console.log('exchAdapter ' + exchAdapter);
			exchAdapter.translateRequest(requestBidPayload);
